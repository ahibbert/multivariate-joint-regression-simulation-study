...
calc_joint_likelihood <- function(input_par,mm_mar,margin_dist,copula_dist,return_option="list",copula_link,dataset,mm_cop,verbose=TRUE,calc_d2=FALSE,dFUN=dZISICHEL,pFUN=pZISICHEL)  {
  ...
  #return_option="list";start_fitted_margins=fitted_margins;input_par=input_par;copula_dist="t";margin_dist = ZISICHEL(mu.link = "log",sigma.link = "log",nu.link = "identity",tau.link = "logit");verbose=TRUE;dFUN=dZISICHEL;pFUN=pZISICHEL;calc_d2=FALSE
  ...
  eta_cop=eta_cop[order(as.numeric(rownames(mm))),]
  eta_cop_link_inv=eta_cop_link_inv[order(as.numeric(rownames(mm))),]
  
  copula_d=dcdu1=dcdu2=dldth=d2ldth=dldz=d2ldz=dthdeta=dzdeta=matrix(0,nrow=nrow(eta_cop),ncol=1)
  
  margin_p_cop_input=matrix(0,nrow=0,ncol=2)
  for (i in 1:(num_margins-1)) {
    margin_p_cop_input  = rbind(margin_p_cop_input,cbind(margin_p[dataset$time %in% margin_names[i]],margin_p[dataset$time %in% margin_names[i+1]]))
  }
  ...
  return_list=list(log_lik_results,num_margins,response,margin_d,margin_p,copula_d,derivatives_copula,margin_dist,copula_dist,eta,eta_linkinv,derivatives_calculated_all_margins,mm_mar,start_par,eta_cop,eta_cop_link_inv,mm_cop)
  names(return_list)=c("log_lik_results","num_margins","response","margin_d","margin_p","copula_d","derivatives_copula","margin_dist","copula_dist","eta","eta_linkinv","derivatives_calculated_all_margins","mm_mar","start_par","eta_cop","eta_cop_link_inv","mm_cop")
...
newton_raphson_iteration=function(results,input_par,phi=1,step_size=1,verbose=c(FALSE,FALSE),calc_d2=FALSE) {
  ...
  #results=results;input_par=input_par;verbose=c(TRUE,TRUE); calc_d2=TRUE
  
  steps_all=list()
  par_out=list()
  margin_dist=results$margin_dist
  copula_dist=results$copula_dist
  d=results$num_margins
  n=length(results$response)/d
  ...
  # Put all copula derivatives in corrrect structure for calculation of marginal likelihood functions
  # Copula derivatives are of the form time1/time2/response so there are n-1 copula derivatives. For the margins, the copula value is for all margins related to that time point summed.
  
  margin_parameters=names(results$mm_mar)
  ...
  # Extract MARGINAL DERIVATIVES from results list (from calc_joint_likelihood)
  dcdu=cbind(dcdu1/(results$copula_d),dcdu2/results$copula_d) #+ (dcdu2)/results$copula_d[,i]  #*(results$margin_d[,i]*results$margin_d[,i+1])
  colnames(dcdu)=c("dcdu1","dcdu2")
  
  #So what we need is: dcdu*x*dfdmu/copula_c
  time_patient_id_cop_t1=time_patient_id_cop_t2=matrix(ncol=2,nrow=0)
  for (i in 1:(d-1)) {
    time_patient_id_cop_t1=rbind(time_patient_id_cop_t1,cbind(i,(1:n)))
    time_patient_id_cop_t2=rbind(time_patient_id_cop_t2,cbind(i+1,(1:n)))
  }
  colnames(time_patient_id_cop_t1)=colnames(time_patient_id_cop_t2) = c("time",'subject')
  
  time_patient_id_mar=dataset[,c("time","subject")]
  
  
  dcdu_merged=merge(  merge(cbind(time_patient_id_cop_t2,dcdu),cbind(time_patient_id_mar,cbind(margin_d,response,dldpar)),by=c("time","subject"),all.x=TRUE)
                      , merge(cbind(time_patient_id_cop_t1,dcdu),cbind(time_patient_id_mar,cbind(margin_d,response,dldpar)),by=c("time","subject"),all.x=TRUE)
                      , by=c("time","subject"),all=TRUE)
  dcdu_merged[is.na(dcdu_merged)] <- 0
  
  dcdu_merged=dcdu_merged[order(dcdu_merged$subject,dcdu_merged$time),]
  
  dlcopdpar=
    dcdu_merged$dcdu1.x*dcdu_merged$margin_d.x*dcdu_merged$response.x*dcdu_merged$dldm.x+
    dcdu_merged$dcdu2.x*dcdu_merged$margin_d.y*dcdu_merged$response.y*dcdu_merged$dldm.y + 
    dcdu_merged$dcdu1.y*dcdu_merged$margin_d.y*dcdu_merged$response.y*dcdu_merged$dldm.y
  #+dcdu_merged$dcdu2.y*dcdu_merged$margin_d.y*dcdu_merged$response.y*dcdu_merged$dldm.y
  ...
  #dlcopdpar_merged=dlcopdpar_merged[order(dlcopdpar_merged$subject,dlcopdpar_merged$time),]
  
  #dlcopdpar_full=matrix(ncol=ncol(dldpar),nrow=nrow(dldpar))
  #for (i in 1:ncol(dldpar)) {
  #  dlcopdpar_full[,i]=(dlcopdpar_merged[,3]+dlcopdpar_merged[,6])*results$response*(dldpar[,i])*results$margin_d
  #}
  
  dlcopdpar_full=cbind(dlcopdpar,dlcopdpar,dlcopdpar,dlcopdpar)
  
  margin_score=score_function(eta=results$eta[,margin_parameters],dldpar=dldpar,dpardeta=dpardeta,dlcopdpar=dlcopdpar_full,response=results$response,phi=phi,step_size=step_size,verbose=verbose[1]) ##Returns updated value
...
#### 2. Load RAND data subset and transform to standard longitudinal dataset #######
...
if (simOption==1) {
  ...
  for ( i in 1:ncol(copsim)) {
    margin[,i]=qZISICHEL(copsim[,i],mu=20,sigma=10,nu=10,tau=0.05)
  }
...
#### 4. Setup model matrices and parameter vector for optimisation ####
...
gamlss_model <- gamlss(formula = mu.formula
                       ...
                       , family=margin_dist,data=dataset)
...
first_input_par=start_par*.8#*(1+(runif(length(start_par))-.5)*.3)
...
while ((abs(change) > .05*phi*step_adjustment) & run_counter <= 100) { #
  ...
  for (par_name in colnames(end_par_matrix)) {
    #plot(1:nrow(end_par_matrix),end_par_matrix[,par_name],main=paste(par_name,sep=""))
    if (end_par[par_name] < start_par[par_name]-0.5*(abs(start_par[par_name])) | end_par[par_name] > start_par[par_name]+0.5*(abs(start_par[par_name]))) {
      plot(1:nrow(end_par_matrix),end_par_matrix[,par_name],main=paste(par_name, " >50% CHANGE",sep=""))
    } else {
      plot(1:nrow(end_par_matrix),end_par_matrix[,par_name],main=paste(par_name, " lims +/- 50% start",sep=""),ylim=c(start_par[par_name]-0.5*(abs(start_par[par_name])),start_par[par_name]+abs(start_par[par_name])*0.5))
    }    
  }
...
#### 6. Post-optimisation calculations ####
