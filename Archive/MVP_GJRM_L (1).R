...
newton_raphson_iteration=function(results,input_par,phi=1,step_size=1,verbose=c(FALSE,FALSE)) {
    ...
    #d2ldpar=cbind(d2ldth,d2ldz)
    dpardeta=cbind(dthdeta,dzdeta)
    dpardeta=dldpar*0+1 ##########Temporary step######## COME BACK TO THIS
    ...
    for (j in 1:ncol(copula_score$z_k)) {
      ...
      if(calc_d2==TRUE) {
        beta_cop_l2[[parameter]]=-t(X)%*%W%*%X  
        rownames(beta_cop_l2[[parameter]])= paste(parameter,rownames(beta_cop_l2[[parameter]]),sep=".")
      }
    }
    ...
    for (parameter in margin_parameters) {
      ...
      if(calc_d2==TRUE) {
        beta_mar_l2[[parameter]]=-t(X)%*%W%*%X  
        rownames(beta_mar_l2[[parameter]])= paste(parameter,rownames(beta_mar_l2[[parameter]]),sep=".")
      }
...
while ((abs(change) > .1*phi | run_counter <= 100) & run_counter <= 10000) {
...
###POST ESTIMATE CALCULATIONS ####

final_results=calc_joint_likelihood(input_par = end_par_matrix[nrow(end_par_matrix),]  #optim_results$par
                                    , mm_mar = mm_mar
                                    , margin_dist = ZISICHEL(
                                      mu.link = "log",
                                      sigma.link = "log",
...
cop_deriv_input=final_results$derivatives_copula
cop_deriv=cbind(cop_deriv_input$d2ldth/final_results$copula_d-2*(exp(cop_deriv_input$dldth)),
                cop_deriv_input$d2ldz/final_results$copula_d-2*(exp(cop_deriv_input$dldz))) ###Based on  DOI 10.1007/s00180-013-0423-8
colnames(cop_deriv)=c("d2ldth","d2ldz")
cop_hess_calc=colMeans(cbind(cop_deriv[,"d2ldth"],cop_deriv[,"d2ldz"]))

mar_deriv=final_results$derivatives_calculated_all_margins[,c("d2ldm2","d2ldd2","d2ldv2","d2ldt2")]
mar_hess_calc=colMeans(mar_deriv[,c("d2ldm2","d2ldd2","d2ldv2","d2ldt2")])

sqrt(solve(diag(-mar_hess_calc)))/sqrt(length(mar_deriv[,"d2ldm2"]))
sqrt(solve(diag(-cop_hess_calc)))/sqrt(length(cop_deriv[,"d2ldth"]))
...
copula_score=score_function(eta=eta_input,dldpar=dldpar,dpardeta=dpardeta,dlcopdpar=dldpar*0,response=dldpar*0,phi=phi,step_size=step_size,verbose=verbose[2]) ##Returns updated value
final_results$mm_mar$mu

mm=final_results$mm_mar$mu
X=as.matrix(mm)
W=diag(copula_score$w_k[,j])
e_k_par=e_k[,j]
beta_new_cop[[parameter_names[j]]]=solve(t(X)%*%W%*%X)%*%t(X)%*%W%*%e_k_par
